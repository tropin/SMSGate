...
namespace Csharper.Common
{
    public class GSMEncoding : System.Text.Encoding
    {
        ...
        public static byte[] Encode7Bit(byte[] source, int udhLength = 0)
        {
            byte[] result = new byte[source.Length - (int)Math.Ceiling(source.Length/8.0)];
            int? padding = null;
            if (udhLength != 0)
                padding = ((udhLength + 1) * 8) % 7;
            int used = 0;
            unsafe
            {
                fixed(byte* pSource = source)
                {
                    ...
                    {
                        SMS_GSMEncode(source.Length, pSource, padding, result.Length, pResult, &used);
                    }
                }
            }
            return result;
        ...
        */
        ...
        SMS_GSMEncode(
           ...
           byte*           outBuff,        /* A pointer to the available octets */
        ...
        )
        {
           ...
           *outUsed = 0;
           ...
           if( paddingBits.HasValue )
           {
              bits = 7 - paddingBits.Value;
              *outBuff++ = (byte)(inText[0] << (7 - bits));
              (*outUsed) ++;
              bits++;
           }
           for( i = 0; i < inSize; i++ )
           {
              ...
              octet = (byte)((inText[i] & 0x7f) >> bits);
              if( i < inSize - 1 )
                 octet |= (byte)(inText[i + 1] << (7 - bits));
              *outBuff++ = octet;
              (*outUsed)++;
              bits++;
           }
